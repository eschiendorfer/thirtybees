 ...
 * @since 1.5
 */
class HTMLTemplateZweiteMahnung extends HTMLTemplate
{
    public $order;
    ...
    public function getContent()
    {
        ...
        if (Configuration::get('PS_PDF_IMG_INVOICE')) {
            foreach ($order_details as &$order_detail) {
                if ($order_detail['image'] != null) {
                    $name = 'product_mini_'.(int)$order_detail['product_id'].'_'.$this->order->id_shop.'.jpg';
		...
		$status = $this->checkstatus($id_order);
		
		foreach ($status as $sta) {
			// Ware verstandt Status
			if ($sta['id_order_state']==4) {
				foreach ($sta as $s) {
				 	$versanddatum = $s;
				}
			}
			// Zahlungserinnerung Status
			if ($sta['id_order_state']==14) {
				foreach ($sta as $s) {
				 	$erinnerungdatum = $s;
				}
			}
		...
		$id_address_invoice = (int)$this->order->id_address_invoice;
		
		$sql_name = 'SELECT firstname, lastname ' .
		' FROM ps_address ' .  
		' WHERE id_address = ' . $id_address_invoice;
				
		$names = Db::getInstance()->ExecuteS($sql_name);
	    ...
	    $preis = number_format(($this->order_invoice->total_paid_tax_incl+10),2,",","");

        $image_path = 'modules/genzo_erp/views/img/barcodes/orders/'.$this->order->id.'.png';
        ...
        $data = array(
			...
			'lastname' => $lastname,
			'firstname' => $firstname,
			'versanddatum' => $versanddatum ?? $this->order->date_add,
			'erinnerungdatum' => $erinnerungdatum ?? '',
			'erste_mahnung' => $erste_mahnung ?? '',
			'price' => $preis,
            'barcode' => _PS_ROOT_DIR_.'/'.$image_path,
            'payment_due_date' => HTMLTemplateInvoice::calculatePaymentDueDate($this->order),
        );
    ...
    protected function getTemplateByCountry($iso_country)
    {
        ...
        if (!$template) {
            $template = $this->getTemplate('zweite-mahnung');
        }
	...
	public function checkstatus($id_order) {
		...
		$status = Db::getInstance()->ExecuteS($sql_status);
