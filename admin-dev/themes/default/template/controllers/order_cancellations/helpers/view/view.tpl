...
<div class="panel">
	<h3><i class="icon-ban"></i> {l s='Order cancellation'}</h3>
	<div class="row form-horizontal">
		<div class="col-lg-6">
			<div class="form-group">
				<label class="control-label col-lg-4">{l s='ID'}</label>
				<div class="col-lg-8">
					<p class="form-control-static">{$order_cancellation->id|intval}</p>
				</div>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				<label class="control-label col-lg-4">{l s='Requested refund method'}</label>
				<div class="col-lg-8">
					<p class="form-control-static">
						{if $refund_method_label}
							{$refund_method_label|escape:'html':'UTF-8'}
						{else}
							<span class="text-muted">-</span>
						{/if}
					</p>
		...
		<div class="col-lg-6">
			<div class="form-group">
				<label class="control-label col-lg-4">{l s='Quoted total'}</label>
				<div class="col-lg-8">
					<p class="form-control-static">
						{if $has_quoted_refund_total}
							{displayWtPriceWithCurrency price=$order_cancellation->quoted_refund_total_tax_incl currency=$currency}
						{else}
							<span class="text-muted">-</span>
						{/if}
					</p>
			...
			<div class="form-group">
				<label class="control-label col-lg-4">{l s='Quoted fee'}</label>
				<div class="col-lg-8">
					<p class="form-control-static">
						{if $has_quoted_fee}
							{displayWtPriceWithCurrency price=$order_cancellation->quoted_fee_tax_incl currency=$currency}
						{else}
							<span class="text-muted">-</span>
						{/if}
					</p>
			...
			<div class="form-group">
				...
				<div class="col-lg-8">
					<p class="form-control-static">{if $order_cancellation->migrated}{l s='Yes'}{else}{l s='No'}{/if}</p>
				</div>
			</div>
			<div class="form-group">
				<label class="control-label col-lg-4">{l s='Date issued'}</label>
				<div class="col-lg-8">
					<p class="form-control-static">{dateFormat date=$order_cancellation->date_add full=true}</p>
				</div>
			</div>
		</div>
	</div>
	...
	<table class="table">
		<thead>
			<tr>
				...
				<th class="text-right">{l s='Cancelled quantity'}</th>
			</tr>
		</thead>
		<tbody>
			{foreach from=$details item=detail}
				<tr>
					...
					<td class="text-right">{$detail.cancelled_quantity|intval}</td>
				</tr>
			{foreachelse}
				<tr>
